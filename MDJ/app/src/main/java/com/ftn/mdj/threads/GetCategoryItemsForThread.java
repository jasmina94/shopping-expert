...
@Getter
public class GetCategoryItemsForThread extends Thread {

    private Handler handler;
    private Handler responseHandler;

    public GetCategoryItemsForThread(Handler handlerUI) {
        this.responseHandler = handlerUI;
        this.handler = new Handler(){

            @Override
            public void handleMessage(Message msg) {
                long categoryId =  msg.getData().getLong("categoryId");
                ServiceUtils.categoryItemService.getItemsForCategory(categoryId).enqueue(new retrofit2.Callback<GenericResponse<List<CategoryItemDTO>>>(){
